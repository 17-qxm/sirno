...
It stores named concepts and their relationships, refines those ideas from broad design to local implementation,
binds them to repository artifacts, and requires re-examination when an upstream concept or relationship changes.
...
A set of named markdown documents, each with a metadata block and a body of prose.
The names are nominal identifiers, providing a canonical reference of the document.
The documents are called *entries* that are readable within five minutes.
The YAML metadata block contains the entry's full name, description, category, and relational information.

Some entries can be categorized as a *concept* or a *narrative*.
Relational information includes *clustee*, *refiner*, and *witness*.
...
Compression is the key to scaling understanding across a codebase and across time.
...
It's rather token-efficient to keep those names as the stable anchors for understanding and change evaluation.

LZ77's algorithm of adaptive dictionary, infamously used in ZIP, is an illustration of how *concepts* should work.
Even better, concepts are human-readable and human-friendly.

Reflecting on aforementioned paradigms:
- Concepts cluster behavioral specifications to reduce cognitive friction.
- Concepts keep intent compressed and portable across levels of detail.
- Concepts categorize test cases so that properties and constraints naturally emerge.

## Interactive Narrative

Understanding is a process, not a state. Concepts may unfold over time, but it takes effort.
A good guide can make that process more efficient and more enjoyable,
which is why we advocate for *interactive narrative*.
...
The narrative should be generated by paragraph after a few positioning questions and spontaneous reflecting quizzes.
It's an ephemeral dialogue that should not be considered part of the canonical documentation,
but it can be a powerful tool for onboarding and knowledge transfer.

## Clustee of a Clique

Tag, scope, namespace, and domain. All these terms point to the same idea: a named clique of related entries.

## Refinement

Refinement then unfolds high-level ideas into lower-level design, implementation, and tests,
without discarding the meaning that made the system coherent in the first place.
